...
class Issue_Book(Resource):
    ...
    def post(self):
        if request.method=='POST':
            return issue.issue_books(request.json)
...
class Initiate_Return_Request(Resource):
    ...
    def patch(self,id):
        if request.method=='PATCH':
            return issue.initiate_return_request(id)
...
class Issued_Book(Resource):
    ...
    def get(self):
        if request.method=='GET':
            ...
            return issue.issued_details(page,per_page,book_id,issue_date,status,student_id)
...
class All_Students_Issued_Details(Resource):
    ...
    def get(self):
        if request.method=='GET':
            ...
            return issue.get_all_students_issued_details(page,per_page,issue_date,status,student_id,book_id,issued_by)
        message = "This method is not allowed here please use the 'GET' method"
        return f"data="", error={True}, code='405', message={message}, details=''"
...
class Issued_Details_History(Resource):
    ...
    def get(self):
        if request.method=='GET':
            ...
            student_id = request.args.get('student_id',type=int)
            issued_by = request.args.get('issued_by',type=int)
            return issue.issued_details_history(page,per_page,book_id,issue_date,student_id,issued_by)
        message = "This method is not allowed here please use the 'GET' method"
        return f"data="", error={True}, code='405', message={message}, details=''"